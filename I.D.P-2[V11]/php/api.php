...
if ($_POST['action'] == 'update_profile') {
    ...
    try {
        ...
        $query = "SELECT id, full_name, email, user_type, phone, division, district, post_code, area, bio, skills, average_rating, total_reviews, created_at FROM users WHERE id = ?";
        $stmt = $db->prepare($query);
...
if ($_POST['action'] == 'complete_work') {
    ...
    // Validate inputs
    if (empty($work_id) || empty($user_id)) {
        sendResponse(false, "Missing required parameters");
    }
    ...
    try {
        ...
        if (!$work) {
            sendResponse(false, "Work not found or you don't have permission to complete this work");
            return;
        }
        
        if ($work['status'] !== 'assigned') {
            sendResponse(false, "Work must be in 'assigned' status to be completed. Current status: " . $work['status']);
            return;
        }
        ...
        // Also update the application status to reflect completion
        $updateAppQuery = "UPDATE applications SET status = 'completed' WHERE work_id = ? AND status = 'accepted'";
        $updateAppStmt = $db->prepare($updateAppQuery);
        $updateAppStmt->execute([$work_id]);
        
        sendResponse(true, "Work marked as completed successfully! You can now review the other party.");
    } catch (Exception $e) {
...
if ($_GET['action'] == 'get_work_details') {
    $database = new Database();
    $db = $database->getConnection();
    ...
    try {
        ...
        if ($work) {
            sendResponse(true, "Work details retrieved", $work);
        } else {
            sendResponse(false, "Work not found");
        }
    } catch (Exception $e) {
        sendResponse(false, "Failed to retrieve work details: " . $e->getMessage());
    }
}
...
if ($_GET['action'] == 'get_user_data') {
    ...
    try {
        $query = "SELECT id, full_name, email, user_type, phone, division, district, post_code, area, bio, skills, average_rating, total_reviews, created_at 
                  FROM users WHERE id = ?";
        $stmt = $db->prepare($query);
        $stmt->execute([$user_id]);
        $user = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        if ($user) {
            sendResponse(true, "User data retrieved", $user);
        } else {
            sendResponse(false, "User not found");
        }
    } catch (Exception $e) {
        sendResponse(false, "Failed to retrieve user data: " . $e->getMessage());
    }
}

// Search users with their reviews
if ($_GET['action'] == 'search_users_with_reviews') {
    $database = new Database();
    $db = $database->getConnection();
    ...
    try {
        if (empty($search_term)) {
            sendResponse(false, "Please enter a search term");
            return;
        }
        
        // Search users
        $query = "SELECT id, full_name, email, user_type, phone, division, district, area, 
        ...
        foreach ($users as &$user) {
            ...
            $user['reviews'] = $reviewStmt->fetchAll(PDO::FETCH_ASSOC);
        }
        
        sendResponse(true, "Users retrieved with reviews", $users);
    } catch (Exception $e) {
        sendResponse(false, "Failed to search users: " . $e->getMessage());
    }
}

// Get user data for search - ADD THIS SECTION
if ($_GET['action'] == 'get_user_data_for_search') {
    $database = new Database();
    $db = $database->getConnection();
    ...
    try {
        $query = "SELECT id, full_name, email, user_type, phone, division, district, area, bio, skills, average_rating, total_reviews, created_at 
                  FROM users WHERE id = ?";
        $stmt = $db->prepare($query);
        $stmt->execute([$user_id]);
        $user = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        if ($user) {
            ...
            sendResponse(true, "User data retrieved with reviews", $user);
        } else {
            sendResponse(false, "User not found");
        }
    } catch (Exception $e) {
...
// Search users with reviews - ENSURE THIS EXISTS
if ($_GET['action'] == 'search_users_with_reviews') {
    $database = new Database();
    $db = $database->getConnection();
