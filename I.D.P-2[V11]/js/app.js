class QuickWorkApp {
    ...
    init() {
        ...
        this.checkAuth();
        this.ensureNavigationLinks(); // Add this line
    }

    // Add this new method
    ensureNavigationLinks() {
        // Ensure Search Users link exists in all navigation menus
        const navMenus = document.querySelectorAll('.nav-menu');
        navMenus.forEach(navMenu => {
            const existingSearchLink = navMenu.querySelector('a[href="user-search.html"]');
            if (!existingSearchLink) {
                const searchUsersItem = document.createElement('li');
                searchUsersItem.innerHTML = '<a href="user-search.html">Search Users</a>';
                
                // Find the profile link and insert before it
                const profileLink = navMenu.querySelector('a[href="profile.html"]');
                if (profileLink && profileLink.parentElement) {
                    profileLink.parentElement.parentElement.insertBefore(searchUsersItem, profileLink.parentElement);
                } else {
                    // If no profile link, just append
                    navMenu.appendChild(searchUsersItem);
                }
            }
            
            // Ensure Completed Work link exists
            const existingCompletedWorkLink = navMenu.querySelector('a[href="completed-work.html"]');
            if (!existingCompletedWorkLink) {
                const completedWorkItem = document.createElement('li');
                completedWorkItem.innerHTML = '<a href="completed-work.html">Completed Work</a>';
                
                // Find the messages link and insert after it
                const messagesLink = navMenu.querySelector('a[href="messages.html"]');
                if (messagesLink && messagesLink.parentElement) {
                    // Insert after messages link
                    if (messagesLink.parentElement.nextSibling) {
                        messagesLink.parentElement.parentElement.insertBefore(completedWorkItem, messagesLink.parentElement.nextSibling);
                    } else {
                        messagesLink.parentElement.parentElement.appendChild(completedWorkItem);
                    }
                }
            }
        });
    ...
    updateNavigation() {
        ...
        if (this.currentUser) {
            ...
            // Add Search Users link to navigation if not already present
            const navMenus = document.querySelectorAll('.nav-menu');
            navMenus.forEach(navMenu => {
                const searchUsersLink = navMenu.querySelector('a[href="user-search.html"]');
                if (!searchUsersLink) {
                    // Find the profile link and insert before it
                    const profileLink = navMenu.querySelector('a[href="profile.html"]');
                    if (profileLink && profileLink.parentElement) {
                ...
                // Add Completed Work link to navigation if not already present
                const completedWorkLink = navMenu.querySelector('a[href="completed-work.html"]');
                if (!completedWorkLink) {
                    // Find the messages link and insert after it
                    const messagesLink = navMenu.querySelector('a[href="messages.html"]');
                    if (messagesLink && messagesLink.parentElement) {
                        const completedWorkItem = document.createElement('li');
                ...
                applicationsList.innerHTML = userPosts.map(post => `
                    <div class="application-item" style="padding: 1rem; border-bottom: 1px solid #e2e8f0;">
                        ...
                        ${post.status === 'assigned' ? `
                            <button class="btn btn-primary" style="margin-top: 0.5rem; background: #10b981; border-color: #10b981;" onclick="app.completeWork(${post.id})">
                                ✅ Mark as Completed
                            </button>
                        ` : ''}
                    </div>
        ...
        container.innerHTML = applications.map(app => `
            <div class="application-card" data-status="${app.status}" data-application-id="${app.id}">
                ...
                <div class="application-actions">
                    ...
                    ${app.status === 'accepted' ? `
                        <button class="btn btn-primary" onclick="app.completeWork(${app.work_id})" style="background: #10b981; border-color: #10b981;">
        ...
        if (modal && details) {
            details.innerHTML = `
                <div class="application-detail-view">
                    ...
                    <div class="detail-actions">
                        <button class="btn btn-primary" onclick="app.completeWork(${application.work_id})" style="background: #10b981; border-color: #10b981;">
                            Mark Work as Completed
                        </button>
                    </div>
                    ` : ''}
    ...
    updateProfileStats() {
        ...
        if (completedJobsElement) {
            ...
            completedJobsElement.textContent = '0';
        }
    }
    ...
    // Mark work as completed - FIXED VERSION
    async completeWork(workId) {
        if (!confirm('Are you sure you want to mark this work as completed?\n\nAfter completion, you will be able to review the other party.')) {
            return;
        }
        
        try {
            // First, verify the work exists and is assigned
            const verifyResponse = await fetch(`php/api.php?action=get_work_details&work_id=${workId}`);
            const verifyResult = await verifyResponse.json();
            
            if (!verifyResult.success) {
                this.showNotification('Work not found or access denied', 'error');
                return;
            }
            
            const work = verifyResult.data;
            
            // Check if work is in assigned status
            if (work.status !== 'assigned') {
                this.showNotification(`Cannot complete work. Current status: ${work.status}. Work must be assigned first.`, 'error');
                return;
            }
            
            // Check if user has permission to complete this work
            const isEmployer = work.employer_id == this.currentUser.id;
            const isWorker = work.assigned_worker_id == this.currentUser.id;
            
            if (!isEmployer && !isWorker) {
                this.showNotification('You do not have permission to complete this work', 'error');
                return;
            }
            ...
            if (result.success) {
                this.showNotification('Work marked as completed successfully! You can now review the other party.', 'success');
                ...
                setTimeout(() => {
                    ...
                    // Redirect to completed work page to submit reviews
                    if (!window.location.pathname.includes('completed-work.html')) {
                        window.location.href = 'completed-work.html';
                    }
                }, 1500);
            ...
            this.showNotification('Failed to complete work. Please try again.', 'error');
        }
    }
